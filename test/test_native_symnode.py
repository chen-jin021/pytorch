    ...
    def test_flag(self):
        ...
        for on in (False, True):
            with torch._dynamo.config.patch(use_cpp_symnode=on):
                env = ShapeEnv()
            want = on and torch._dynamo.config.use_cpp_fake_tensor
            self.assertEqual(env._native_env is not None, want)
        with torch._dynamo.config.patch(
            use_cpp_symnode=True, use_cpp_fake_tensor=False
        ):
            self.assertIsNone(ShapeEnv()._native_env)
...
class TestNativeSymNodeCompile(TestCase):
    ...
    def run_compiled(self, case, native):
        ...
        torch._dynamo.reset()
        with torch._dynamo.config.patch(
            use_cpp_symnode=native, use_cpp_fake_tensor=True
        ):
